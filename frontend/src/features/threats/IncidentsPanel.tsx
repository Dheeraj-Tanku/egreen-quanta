...
export function IncidentsPanel() {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {COLUMNS.map((col) => (
        <div key={col} className="rounded-xl border border-border bg-surface/40 p-2">
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {COLUMNS.filter((c) => c !== inc.status).map((c) => (
                      <button
                        ...
                        className="rounded bg-surface-2 px-1.5 py-0.5 text-[10px] text-muted hover:text-fg"
                      >
                        → {c}
                      </button>
                    ))}
