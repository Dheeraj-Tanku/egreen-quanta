...
export function ThemeToggle() {
  ...
  return (
    <div className="flex items-center rounded-lg border border-border bg-surface-2 p-0.5" role="group" aria-label="Theme">
      {OPTIONS.map((o) => (
        <button
          key={o.value}
          ...
          className={cn(
            ...
            theme === o.value ? "bg-surface text-fg" : "text-muted hover:text-fg",
          )}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} className="h-[14px] w-[14px]" aria-hidden="true">
