...
export function Sidebar() {
  return (
    <aside className="hidden w-56 shrink-0 flex-col border-r border-border bg-surface/60 px-3 py-4 md:flex">
      <Brand />
      <NavList />
      <div className="mt-auto px-2 pt-4 text-[10px] text-muted">v0.1.0 · PS-141 · SIH 2026</div>
    </aside>
  );
}
...
export function MobileNav({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <Drawer open={open} onClose={onClose} title="Navigation" width="15rem" side="left">
